...
const App = (function() {
    ...
    const loadEventListeners = function() {
        ...
        // delete item
        document.querySelector(UISelectors.itemsContainer).addEventListener("click", deletedItem)
    ...
    const addIncome = function() {
        ...
        if(description.descriptionInput !== "" && amount.amountInput !== "") {
            // add new item
            const newMoney = itemControl.addMoney(description.descriptionInput, amount.amountInput);
    ...
    const deletedItem = function(e)  {
        e.preventDefault()
        	
        // deletedItem.remove()
        if(e.target.classList.contains("far")) {
            // get id number
            const id = itemControl.getIdNumber(e.target)
            // delete amount from ui
            UICtrl.deleteAmount(id);
            // delete amount from  data
            itemControl.deleteAmountArr(id);
            // update total spents
            UICtrl.updateSpent();
            // calculate money available
            UICtrl.updateAvailable();
